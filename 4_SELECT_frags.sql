...
JOIN data_time_frag t ON cdu.time_id = t.id
JOIN segment_frag s ON cdu.segment_id = s.id
JOIN vertice_frag v ON s.start_vertice_id = v.id
JOIN district_frag d ON v.district_id = d.id
WHERE
    d.name = 'IGUATEMI'
    AND t.year = 2016
GROUP BY
...
FROM
    crime_frag c
JOIN data_time_frag t ON c.time_id = t.id
JOIN segment_frag s ON c.segment_id = s.id
JOIN vertice_frag v ON s.start_vertice_id = v.id
JOIN neighborhood_frag n ON v.neighborhood_id = n.id
WHERE
